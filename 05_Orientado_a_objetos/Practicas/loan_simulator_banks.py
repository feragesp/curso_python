...
class LoanSimulator:
    def __init__(self, detalles_prestamo, años=30, precio_vivienda=300000):
        ...
        self._tasa_interes = detalles_prestamo.imag # Tasa de interes del prestamo
        self._años = años # Años del prestamo
        self._precio_vivienda = precio_vivienda # Precio de la vivienda
        #----------------#
        self._tasa_mensual = (self._tasa_interes / 12) / 100 # Tasa de interes anual => Mensual
        self._numero_pagos = (self._años * 12) # Duración del prestamos en meses
        self._prestamo = self._precio_vivienda - self._entrada # Prestamo de la vivienda
        self._cuota_mensual = 0 # Simulacion de la cuota mensual

    def calcular_pago_total(self):
        """
        Calcula la cuota mensual del préstamo según la tasa de interés y número de pagos
        """
        self._cuota_mensual = self._prestamo * \
        (self._tasa_mensual * (1 + self._tasa_mensual)**self._numero_pagos) / \
        ((1 + self._tasa_mensual)**self._numero_pagos - 1)

    def mostrar_resultado(self):
        """
        Imprime los resultados de la simulación de la hipoteca 
        (Precio Vivienda, Entrada, Tasa de interés, duración y cuota mensual)
        """
        print("-"*5 + " Simulación Hipoteca " + "-"*5)
        print(f"Para una vivienda de {self._precio_vivienda} €, \
              \naportando una entrada de {self._entrada} € \
              \ny con una tasa de interés del {self._tasa_interes}% \
              \nanual durante {self._años} años:")
        print(f"Cuota mensual a pagar: {self._cuota_mensual} €")
        print("-"*5 + " Fin de la Simulación " + "-"*5)
...
class Bbva(LoanSimulator):
    def __init__(self, entrada, años=30, precio_vivienda=300000):
